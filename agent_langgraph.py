...
class Agent:
    ...
    def _build_system(self, user_input, persona=None, style=None):
        ...
        # RAG（方案A·关键词检索，零新依赖）：对话前自动召回相关文档片段注入上下文，
        # 让普通对话也能直接吃知识库；命中才注入并累加「被引用次数」，避免噪声。
        if user_input:
            try:
                hits = retrieve_for_injection(user_input, top_k=4)
                if hits:
                    increment_doc_refs([h["doc_id"] for h in hits])
                    lines = []
                    for h in hits:
                        bc = f"  [{h['headings']}]" if h["headings"] else ""
                        lines.append(
                            f"· 《{h['doc_name']}》(相关度 {h['score']:.2f}){bc}\n  {h['text']}"
                        )
                    system_content += (
